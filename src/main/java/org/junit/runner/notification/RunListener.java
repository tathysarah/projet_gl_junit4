...
/**
 * Register an instance of this class with {@link RunNotifier} to be notified of
 * events that occur during a test run. All of the methods in this class are
 * abstract and have no implementation; override one or more methods to receive
 * events.
 * <p>
 * For example, suppose you have a <code>Cowbell</code> class that you want to
 * make a noise whenever a test fails. You could write:
 * 
 * <pre>
 ...
 * have their {@link RunListener#testFailure(Failure)} called with a
 * {@code Description} of {@link Description#TEST_MECHANISM} to indicate the
 * failure.
 * <p>
 ...
 * multiple threads when tests are run in parallel, you can annotate your test
 * class with {@link RunListener.ThreadSafe}.
 * <p>
 * Listener methods will be called from the same thread as is running the test,
 * unless otherwise indicated by the method Javadoc
 *
...
public class RunListener {
    ...
    /**
     * Called before any tests have been run. This may be called on an arbitrary
     * thread.
     *
     * @param description
    ...
    /**
     * Called when all tests have finished. This may be called on an arbitrary
     * thread.
     *
     * @param result
    ...
    /**
     * Called when a test suite is about to be started. If this method is called
     * for a given {@link Description}, then
     * {@link #testSuiteFinished(Description)} will also be called for the same
     * {@code Description}.
     *
     * <p>
     * Note that not all runners will call this method, so runners should be
     * prepared to handle {@link #testStarted(Description)} calls for tests
     * where there was no corresponding {@code testSuiteStarted()} call for the
    ...
    /**
     * Called when a test suite has finished, whether the test suite succeeds or
     * fails. This method will not be called for a given {@link Description}
     * unless {@link #testSuiteStarted(Description)} was called for the
     * same @code Description}.
     *
     * @param description
