 ...
 *
 * <p>
 * There are four slightly different validators. The
 * {@link #CLASS_RULE_VALIDATOR} validates fields with a {@link ClassRule}
 * annotation and the {@link #RULE_VALIDATOR} validates fields with a
 * {@link Rule} annotation.
 * </p>
 *
 * <p>
 * The {@link #CLASS_RULE_METHOD_VALIDATOR} validates methods with a
 * {@link ClassRule} annotation and the {@link #RULE_METHOD_VALIDATOR} validates
 * methods with a {@link Rule} annotation.
 * </p>
 */
public class RuleMemberValidator {
     ...
     */
    public static final RuleMemberValidator CLASS_RULE_VALIDATOR = classRuleValidatorBuilder()
            .withValidator(new DeclaringClassMustBePublic())
            .withValidator(new MemberMustBeStatic())
            .withValidator(new MemberMustBePublic())
            .withValidator(new FieldMustBeATestRule()).build();

    /**
     * Validates fields with a {@link Rule} annotation.
     */
    public static final RuleMemberValidator RULE_VALIDATOR = testRuleValidatorBuilder()
            .withValidator(new MemberMustBeNonStaticOrAlsoClassRule())
            .withValidator(new MemberMustBePublic())
            .withValidator(new FieldMustBeARule()).build();

    /**
     * Validates methods with a {@link ClassRule} annotation.
     */
    public static final RuleMemberValidator CLASS_RULE_METHOD_VALIDATOR = classRuleValidatorBuilder()
            .forMethods().withValidator(new DeclaringClassMustBePublic())
            .withValidator(new MemberMustBeStatic())
            .withValidator(new MemberMustBePublic())
            .withValidator(new MethodMustBeATestRule()).build();

    /**
     * Validates methods with a {@link Rule} annotation.
     */
    public static final RuleMemberValidator RULE_METHOD_VALIDATOR = testRuleValidatorBuilder()
            .forMethods()
            .withValidator(new MemberMustBeNonStaticOrAlsoClassRule())
            .withValidator(new MemberMustBePublic())
            .withValidator(new MethodMustBeARule()).build();
    ...
    public void validate(TestClass target, List<Throwable> errors) {
        List<? extends FrameworkMember<?>> members = methods
     ...
     * {@link org.junit.Rule} and {@link org.junit.ClassRule} annotations have
     * been used correctly
     */
    interface RuleValidator {
        /**
         * Examine the given member and add any violations of the strategy's
         * validation logic to the given list of errors
         * 
         * @param member
         *            The member (field or member) to examine
         * @param annotation
         *            The type of rule annotation on the member
         * @param errors
    ...
    private static final class MemberMustBeNonStaticOrAlsoClassRule
        ...
        public void validate(FrameworkMember<?> member,
            ...
            boolean isClassRuleAnnotated = (member
                    .getAnnotation(ClassRule.class) != null);
            ...
            if (member.isStatic()
                    && (isMethodRuleMember || !isClassRuleAnnotated)) {
                String message;
     ...
     * {@link org.junit.rules.MethodRule} or {@link org.junit.rules.TestRule}
     */
