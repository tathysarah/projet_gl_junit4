...
public class MethodSorter {
     ...
     * {@link Method#toString()} as a tiebreaker
     */
     ...
     *
     * Using the JVM order is unwise since the Java platform does not specify
     * any particular order, and in fact JDK 7 returns a more or less random
     * order; well-written test code would not assume any order, but some does,
     * and a predictable failure is better than a random failure on certain
     * platforms. By default, uses an unspecified but deterministic order.
     *
     * @param clazz
     *            a class
    ...
    public static Method[] getDeclaredMethods(Class<?> clazz) {
        Comparator<Method> comparator = getSorter(
                clazz.getAnnotation(FixMethodOrder.class));
