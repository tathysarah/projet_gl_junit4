...
public final class Throwables {
    ...
    /**
     * Rethrows the given {@code Throwable}, allowing the caller to declare that
     * it throws {@code Exception}. This is useful when your callers have
     * nothing reasonable they can do when a {@code Throwable} is thrown. This
     * is declared to return {@code Exception} so it can be used in a
     * {@code throw} clause:
     * 
     * <pre>
     * try {
    ...
    /**
     * Gets a trimmed version of the stack trace of the given exception. Stack
     * trace elements that are below the test method are filtered out.
     *
     * @return a trimmed stack trace, or the original trace if trimming wasn't
     *         possible
     */
    public static String getTrimmedStackTrace(Throwable exception) {
        List<String> trimmedStackTraceLines = getTrimmedStackTraceLines(
                exception);
    ...
    private static List<String> getTrimmedStackTraceLines(Throwable exception) {
        List<StackTraceElement> stackTraceElements = Arrays
                .asList(exception.getStackTrace());
        int linesToInclude = stackTraceElements.size();

        State state = State.PROCESSING_OTHER_CODE;
        for (StackTraceElement stackTraceElement : asReversedList(
                stackTraceElements)) {
            ...
            if (state == State.DONE) {
                List<String> trimmedLines = new ArrayList<String>(
                        linesToInclude + 2);
                trimmedLines.add("");
                for (StackTraceElement each : stackTraceElements.subList(0,
                        linesToInclude)) {
                    trimmedLines.add("\tat " + each);
                }
                if (exception.getCause() != null) {
                    trimmedLines.add("\t... "
                            + (stackTraceElements.size() - trimmedLines.size())
                            + " trimmed");
                }
                return trimmedLines;
            }
    ...
    private static List<String> getCauseStackTraceLines(Throwable exception) {
        if (exception.getCause() != null || hasSuppressed(exception)) {
            ...
            BufferedReader reader = new BufferedReader(new StringReader(
                    fullTrace.substring(exception.toString().length())));
            ...
            try {
                ...
                // We should never get here, because we are reading from a
                // StringReader
            }
        }
    ...
    private static void appendStackTraceLines(List<String> stackTraceLines,
    ...
    private enum State {
        ...
        PROCESSING_REFLECTION_CODE {
            ...
            public State processLine(String methodName) {
                if (isReflectionMethod(methodName)) {
                    ...
                    // This is here to handle TestCase.runBare() calling
                    // TestCase.runTest().
                    return PROCESSING_TEST_FRAMEWORK_CODE;
                }
                return DONE;
        ...
        /**
         * Processes a stack trace element method name, possibly moving to a new
         * state.
         */
        ...
        public final State processStackTraceElement(StackTraceElement element) {
            return processLine(element.getClassName() + "."
    ...
    private static final String[] TEST_FRAMEWORK_METHOD_NAME_PREFIXES = {
            ...
            "org.junit.experimental.runners.", "org.junit.internal.",
    ...
    private static boolean isTestFrameworkMethod(String methodName) {
        return isMatchingMethod(methodName, TEST_FRAMEWORK_METHOD_NAME_PREFIXES)
                && !isMatchingMethod(methodName,
                        TEST_FRAMEWORK_TEST_METHOD_NAME_PREFIXES);
    }

    private static final String[] REFLECTION_METHOD_NAME_PREFIXES = {
            "sun.reflect.", "java.lang.reflect.", "jdk.internal.reflect.",
            "org.junit.rules.RunRules.<init>(",
            "org.junit.rules.RunRules.applyAll(", // calls TestRules
            "org.junit.runners.RuleContainer.apply(", // calls MethodRules &
                                                      // TestRules
            "junit.framework.TestCase.runBare(", // runBare() directly calls
                                                 // setUp() and tearDown()
    };
    ...
    private static boolean isMatchingMethod(String methodName,
