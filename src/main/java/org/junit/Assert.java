...
public class Assert {
    ...
    private static boolean doubleIsDifferent(double d1, double d2,
            double delta) {
    ...
    public static void assertEquals(String message, long expected,
            long actual) {
        if (expected != actual) {
            failNotEquals(message, Long.valueOf(expected),
                    Long.valueOf(actual));
        }
    }
    ...
    public static <T extends Throwable> T assertThrows(String message,
        ...
        try {
            ...
            if (expectedThrowable.isInstance(actualThrown)) {
                ...
                Class<? extends Throwable> actualThrowable = actualThrown
                        .getClass();
                String actual = formatClass(actualThrowable);
                if (expected.equals(actual)) {
                    // There must be multiple class loaders. Add the identity
                    // hash code so the message
                    // doesn't say "expected:
                    // java.lang.String<my.package.MyException> ..."
                    expected += "@" + Integer.toHexString(
                            System.identityHashCode(expectedThrowable));
                    actual += "@" + Integer.toHexString(
                            System.identityHashCode(actualThrowable));
                }
                String mismatchMessage = buildPrefix(message) + format(
                        "unexpected exception type thrown;", expected, actual);

                // The AssertionError(String, Throwable) ctor is only available
                // on JDK7.
        ...
        String notThrownMessage = buildPrefix(message) + String.format(
